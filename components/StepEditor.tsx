...
interface StepEditorProps {
  ...
  onAddSections?: (sections: SectionContent[]) => void;
}
...
const StepEditor: React.FC<StepEditorProps> = ({
  ...
  // --- Quick Edit: upload multiple files as sections ---
  const handleQuickUploadFiles = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files;
    if (!files || !onAddSections) return;
    ...
    for (const file of Array.from(files) as File[]) {
      try {
        ...
        addToast('error', `Lỗi đọc file: ${file.name}`);
      }
    }
  ...
  // --- Quick Edit: add manual section ---
  const handleAddManualSection = () => {
    if (!quickMeasureName.trim() || !onAddSections) return;
    onAddSections([{
      id: `qe-${Date.now()}-${Math.random().toString(36).substr(2, 4)}`,
      title: quickMeasureName.trim(),
      level: 1,
      originalContent: '',
      refinedContent: '',
      isProcessing: false,
  ...
  // --- Quick Edit: upload ref docs ---
  const handleQuickRefDocUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files;
    if (!files) return;
    for (const file of Array.from(files) as File[]) {
      try {
        ...
        const newDoc = {
          ...
          name: file.name,
          content: content.substring(0, 15000),
          type: (isExercise ? 'exercise' : 'document') as 'exercise' | 'document'
        };
        onUpdateRequirements({
          ...userRequirements,
          referenceDocuments: [...userRequirements.referenceDocuments, newDoc]
        ...
        console.error('Error reading ref doc:', err);
        addToast('error', `Lỗi đọc tài liệu: ${(file as File).name}`);
      }
    }
    ...
    if (quickEditMode) {
      return (
        <div className="animate-fade-in" style={{
          maxWidth: 680, margin: '0 auto', padding: '24px 0',
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{
              ...
              boxShadow: '0 4px 0 #0f766e, 0 8px 20px rgba(13, 148, 136, 0.25)'
            }}>
              <Pencil size={24} color="white" />
            </div>
            <h2 style={{ fontSize: 24, fontWeight: 800, color: '#134e4a', margin: 0 }}>Sửa từng biện pháp</h2>
          ...
          <div style={{
            ...
            <label style={{ fontSize: 14, fontWeight: 700, color: '#0f766e', display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
              📄 Nội dung biện pháp
            ...
            <div
              ...
              style={{
                border: '2px dashed #99f6e4', borderRadius: 12, padding: '28px 16px',
                textAlign: 'center', cursor: 'pointer', background: '#f0fdfa',
                transition: 'all 0.2s'
              }}
              onMouseEnter={e => { (e.target as HTMLElement).style.borderColor = '#14b8a6'; (e.target as HTMLElement).style.background = '#ecfdf5'; }}
              onMouseLeave={e => { (e.target as HTMLElement).style.borderColor = '#99f6e4'; (e.target as HTMLElement).style.background = '#f0fdfa'; }}
            >
              <Upload size={28} color="#14b8a6" style={{ marginBottom: 8, opacity: 0.7 }} />
              <p style={{ fontSize: 14, fontWeight: 600, color: '#0f766e', margin: 0 }}>Nhấn để chọn file biện pháp</p>
              <p style={{ fontSize: 11, color: '#94a3b8', margin: '4px 0 0' }}>.docx, .pdf, .txt — Chọn nhiều file cùng lúc</p>
            </div>
            <input ref={quickUploadRef} type="file" accept=".docx,.pdf,.txt" multiple style={{ display: 'none' }} onChange={handleQuickUploadFiles} />

            {/* Manual add */}
            <div style={{ display: 'flex', gap: 6, marginTop: 12 }}>
              <input
                type="text"
                placeholder="Hoặc nhập tên biện pháp thủ công..."
                value={quickMeasureName}
                onChange={e => setQuickMeasureName(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && handleAddManualSection()}
                style={{
              ...
              <button
                ...
                disabled={!quickMeasureName.trim()}
                style={{ fontSize: 12, gap: 4, padding: '8px 14px' }}
              >
                <Plus size={14} /> Thêm
              </button>
            </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
              <button
                onClick={() => quickRefDocRef.current?.click()}
                className="btn-secondary btn-sm"
                style={{ fontSize: 12, gap: 4 }}
              >
                <Upload size={13} /> Upload tài liệu
              </button>
              <input ref={quickRefDocRef} type="file" accept=".docx,.pdf,.txt" multiple style={{ display: 'none' }} onChange={handleQuickRefDocUpload} />
            </div>
            ...
            {userRequirements.referenceDocuments.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {userRequirements.referenceDocuments.map(doc => (
                  <div key={doc.id} style={{
                    ...
                    background: doc.type === 'exercise' ? '#fffbeb' : '#f0f9ff',
                    ...
                    <span style={{ color: '#94a3b8', fontSize: 10 }}>{(doc.content.length / 1000).toFixed(1)}k chữ</span>
                    <button onClick={() => handleRemoveRefDoc(doc.id)} style={{ border: 'none', background: 'none', cursor: 'pointer', padding: 2 }}>
              ...
              <p style={{ fontSize: 12, color: '#94a3b8', fontStyle: 'italic', margin: 0 }}>
                Upload tài liệu để AI tham khảo khi phân tích (bài tập, đề thi, giáo án...)
              </p>
            )}
          </div>
          ...
          <div style={{
            ...
            <textarea
              placeholder="VD: Lấy chính xác bài tập ở tài liệu tham khảo, viết chi tiết hơn, thêm ví dụ minh họa..."
              ...
              style={{
                width: '100%', minHeight: 70, padding: '10px 14px', borderRadius: 10,
